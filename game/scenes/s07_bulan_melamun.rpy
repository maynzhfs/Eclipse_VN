label s07_bulan_melamun:
    
    show bulan normal at pos_left with chara_move_transition
    'Bulan sedang melamun di kamarnya. Ponselnya bergetar, ada pesan masuk dari Bumi.'
    
    show bulan normal at pos_far_left with chara_move_transition
    ...
    call talk('bulan', 'Ja, aku bingung, ini kenapa Bumi jadi kayak begini? padahal aku udah nurutin apa yang dia mau')
    
    call talk('senja', 'Dia itu cemburu karena kamu sekarang dengan Hari, Bul')
    
    call talk('bulan', 'Tapi kan itu ga ada hubungannya dengan dia, Ja')
    
    call talk('senja', 'Sudah ga apa Bul, ga usah terlalu dipikirkan. tapi ngomong-ngomong kamu dengan Hari gimana?')
    
    call talk('bulan', 'Kalau aku dengan Hari mah aman-aman aja Ja, cuman aja Bumi agak..')
    
    call talk('senja', 'Yang paling penting kamu bahagia Bul, jadi jangan pikirkan yang tidak perlu kamu pikirin, ok?')
    
    call talk('bulan', 'Senja, kalau besok kita ketemuan di cafe gimana? aku butuh banget kamu sekarang')
    
    call talk('senja', 'ok boleh ko Bul, kita ketemuan aja ya di cafe besok')
    
    jump s08_ngobrol_di_kafe